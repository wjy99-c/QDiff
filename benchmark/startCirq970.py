...
def make_circuit(n: int, input_qubit):
    ...
    for i in range(2):
        ...
        c.append(cirq.CZ.on(input_qubit[2],input_qubit[3])) # number=38
        ...
        c.append(cirq.rx(2.5761059759436304).on(input_qubit[3])) # number=32
        c.append(cirq.H.on(input_qubit[3]))  # number=20

        c.append(cirq.H.on(input_qubit[0]))  # number=33
        c.append(cirq.CZ.on(input_qubit[1],input_qubit[0]))  # number=34
        c.append(cirq.H.on(input_qubit[0]))  # number=35
        c.append(cirq.rx(-0.7822565707438585).on(input_qubit[2])) # number=31
        c.append(cirq.X.on(input_qubit[0]))  # number=29
        c.append(cirq.CNOT.on(input_qubit[1],input_qubit[0]))  # number=30
        c.append(cirq.CNOT.on(input_qubit[0],input_qubit[1]))  # number=25
        c.append(cirq.X.on(input_qubit[1]))  # number=26
...
if __name__ == '__main__':
    ...
    writefile = open("../data/startCirq970.csv","w+")
