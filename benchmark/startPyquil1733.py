# qubit number=4
# total number=47
import pyquil
from pyquil.api import local_forest_runtime, QVMConnection
from pyquil import Program, get_qc
from pyquil.gates import *
import numpy as np

conn = QVMConnection()

def make_circuit()-> Program:

    prog = Program() # circuit begin

    prog += H(0) # number=1

    prog += H(1) # number=2
    prog += H(2) # number=3
    prog += RX(0.7099999397112932,2) # number=10
    prog += H(3)  # number=4
    prog += H(0) # number=11
    prog += H(1) # number=43
    prog += CZ(3,0) # number=12
    prog += H(1) # number=22
    prog += H(0) # number=13
    prog += H(0) # number=7
    prog += CZ(3,0) # number=8
    prog += H(0) # number=9
    prog += H(2) # number=23
    prog += CZ(0,2) # number=24
    prog += H(2) # number=25
    prog += CNOT(0,2) # number=26
    prog += CNOT(0,2) # number=37
    prog += CNOT(0,2) # number=44
    prog += X(2) # number=45
    prog += CNOT(0,2) # number=46
    prog += CNOT(0,2) # number=39
    prog += H(2) # number=29
    prog += CZ(0,2) # number=30
    prog += H(2) # number=31
    prog += CNOT(0,2) # number=21
    prog += H(2) # number=32
    prog += CZ(0,2) # number=33
    prog += H(2) # number=34
    prog += CNOT(0,2) # number=40
    prog += X(2) # number=41
    prog += CNOT(0,2) # number=42
    prog += CNOT(0,2) # number=18
    prog += X(2) # number=35
    prog += X(2) # number=36
    # circuit end

    return prog

def summrise_results(bitstrings) -> dict:
    d = {}
    for l in bitstrings:
        if d.get(l) is None:
            d[l] = 1
        else:
            d[l] = d[l] + 1

    return d

if __name__ == '__main__':
    prog = make_circuit()
    qvm = get_qc('4q-qvm')

    results = qvm.run_and_measure(prog,1024)
    bitstrings = np.vstack([results[i] for i in qvm.qubits()]).T
    bitstrings = [''.join(map(str, l)) for l in bitstrings]
    writefile = open("../data/startPyquil1733.csv","w")
    print(summrise_results(bitstrings),file=writefile)
    writefile.close()

