...
def make_circuit(n: int, input_qubit):
    ...
    c.append(cirq.Z.on(input_qubit[2])) # number=17
    c.append(cirq.H.on(input_qubit[1])) # number=24
    c.append(cirq.CZ.on(input_qubit[2],input_qubit[1])) # number=25
    c.append(cirq.H.on(input_qubit[1])) # number=26
    c.append(cirq.Z.on(input_qubit[1])) # number=21
    c.append(cirq.H.on(input_qubit[1]))  # number=2
    c.append(cirq.H.on(input_qubit[2]))  # number=3
    c.append(cirq.rx(-2.0106192982974678).on(input_qubit[2])) # number=28
    ...
    c.append(cirq.CNOT.on(input_qubit[1],input_qubit[3])) # number=23
...
if __name__ == '__main__':
    ...
    writefile = open("../data/startCirq1649.csv","w+")
