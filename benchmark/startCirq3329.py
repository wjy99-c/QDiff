...
def make_circuit(n: int, input_qubit):
    ...
    c.append(cirq.Z.on(input_qubit[3])) # number=27
    c.append(cirq.H.on(input_qubit[0])) # number=29
    c.append(cirq.CZ.on(input_qubit[3],input_qubit[0])) # number=30
    ...
    c.append(cirq.Y.on(input_qubit[1])) # number=37
    c.append(cirq.H.on(input_qubit[3]))  # number=25
    c.append(cirq.X.on(input_qubit[3]))  # number=18
    c.append(cirq.CNOT.on(input_qubit[3],input_qubit[1])) # number=40
    ...
    c.append(cirq.Y.on(input_qubit[2])) # number=38
    c.append(cirq.Y.on(input_qubit[2])) # number=39
    # circuit end
...
if __name__ == '__main__':
    ...
    writefile = open("../data/startCirq3329.csv","w+")
