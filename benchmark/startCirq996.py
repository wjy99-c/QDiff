...
def make_circuit(n: int, input_qubit):
    ...
    c.append(cirq.CNOT.on(input_qubit[4],input_qubit[0])) # number=30
    c.append(cirq.H.on(input_qubit[0])) # number=36
    c.append(cirq.CZ.on(input_qubit[4],input_qubit[0])) # number=37
    c.append(cirq.H.on(input_qubit[0])) # number=38
    c.append(cirq.CNOT.on(input_qubit[4],input_qubit[0])) # number=40
    c.append(cirq.Z.on(input_qubit[4])) # number=41
    c.append(cirq.CNOT.on(input_qubit[4],input_qubit[0])) # number=42
    c.append(cirq.CNOT.on(input_qubit[4],input_qubit[0])) # number=35
    c.append(cirq.CNOT.on(input_qubit[4],input_qubit[0])) # number=32
    c.append(cirq.H.on(input_qubit[4]))  # number=21
    for i in range(2):
        c.append(cirq.H.on(input_qubit[0]))  # number=1
        c.append(cirq.H.on(input_qubit[1]))  # number=2
        c.append(cirq.H.on(input_qubit[2]))  # number=7
        c.append(cirq.CNOT.on(input_qubit[1],input_qubit[2])) # number=39
...
if __name__ == '__main__':
    ...
    writefile = open("../data/startCirq996.csv","w+")
