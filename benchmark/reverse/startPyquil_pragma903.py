# qubit number=5
# total number=22
import pyquil
from pyquil.api import local_forest_runtime, QVMConnection
from pyquil import Program, get_qc
from pyquil.gates import *
import numpy as np

conn = QVMConnection()

def make_circuit()-> Program:

    prog = Program('PRAGMA INITIAL_REWIRING "NAIVE"') # circuit begin

    prog += Y(0) # number=3
    prog += H(1) # number=4
    prog += RX(3.0096457621390216,3) # number=16
    prog += X(0) # number=17
    prog += CNOT(0,1) # number=19
    prog += Z(0) # number=20
    prog += CNOT(0,1) # number=21
    prog += X(2) # number=5
    prog += CNOT(0,3) # number=8
    prog += CNOT(0,3) # number=12
    prog += X(3) # number=13
    prog += CNOT(0,3) # number=14
    prog += CNOT(0,3) # number=10
    prog += H(4) # number=7
    prog += CNOT(1,3) # number=1
    prog += RX(1.8912387774610557,1) # number=18
    prog += H(4) # number=11
    prog += CNOT(1,2) # number=2

    prog += CNOT(1,2) # number=2
    prog += H(4) # number=11
    prog += RX(1.8912387774610557,1) # number=18
    prog += CNOT(1,3) # number=1
    prog += H(4) # number=7
    prog += CNOT(0,3) # number=10
    prog += CNOT(0,3) # number=14
    prog += X(3) # number=13
    prog += CNOT(0,3) # number=12
    prog += CNOT(0,3) # number=8
    prog += X(2) # number=5
    prog += CNOT(0,1) # number=21
    prog += Z(0) # number=20
    prog += CNOT(0,1) # number=19
    prog += X(0) # number=17
    prog += RX(3.0096457621390216,3) # number=16
    prog += H(1) # number=4
    prog += Y(0) # number=3
    # circuit end

    return prog

def summrise_results(bitstrings) -> dict:
    d = {}
    for l in bitstrings:
        if d.get(l) is None:
            d[l] = 1
        else:
            d[l] = d[l] + 1

    return d

if __name__ == '__main__':
    prog = make_circuit()
    qvm = get_qc('5q-qvm')

    results = qvm.run_and_measure(prog,1024)
    bitstrings = np.vstack([results[i] for i in qvm.qubits()]).T
    bitstrings = [''.join(map(str, l)) for l in bitstrings]
    writefile = open("../data/reverse/startPyquil_pragma903.csv","w")
    print(summrise_results(bitstrings),file=writefile)
    writefile.close()

