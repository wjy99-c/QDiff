...
def make_circuit(n:int) -> QuantumCircuit:
    ...
    prog.cx(input_qubit[3],input_qubit[0]) # number=16
    prog.z(input_qubit[3]) # number=17
    prog.cx(input_qubit[3],input_qubit[0]) # number=18
    prog.h(input_qubit[1]) # number=4
    prog.x(input_qubit[2]) # number=5
    prog.x(input_qubit[3]) # number=6
    prog.z(input_qubit[0]) # number=12
...
if __name__ == '__main__':
    ...
    writefile = open("../data/reverse/startQiskit_pragma846.csv","w")
