...
def make_circuit(n: int, input_qubit):
    ...
    c.append(cirq.rx(2.243097154663112).on(input_qubit[4])) # number=18
    c.append(cirq.H.on(input_qubit[1])) # number=4
    c.append(cirq.H.on(input_qubit[2])) # number=19
    c.append(cirq.CZ.on(input_qubit[0],input_qubit[2])) # number=20
    c.append(cirq.H.on(input_qubit[2])) # number=21
    c.append(cirq.rx(1.5205308443374599).on(input_qubit[2])) # number=15
    c.append(cirq.X.on(input_qubit[2])) # number=13
    ...
    c.append(cirq.Z.on(input_qubit[0])) # number=17
    c.append(cirq.X.on(input_qubit[4])) # number=16
...
if __name__ == '__main__':
    ...
    writefile = open("../data/reverse/startCirq_pragma947.csv","w+")
