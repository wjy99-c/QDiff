...
def make_circuit(n: int, input_qubit):
    ...
    c.append(cirq.CNOT.on(input_qubit[3],input_qubit[0])) # number=52
    c.append(cirq.Z.on(input_qubit[3])) # number=53
    c.append(cirq.CNOT.on(input_qubit[3],input_qubit[0])) # number=54
    for i in range(2):
        ...
        c.append(cirq.rx(-1.928937889304133).on(input_qubit[2])) # number=49
        ...
        c.append(cirq.Z.on(input_qubit[3])) # number=50
...
if __name__ == '__main__':
    ...
    writefile = open("../data/startCirq1910.csv","w+")
